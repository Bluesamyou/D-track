...
class Form extends Component {
    ...
    verifyMake = () => {
        if (this.state.manual) {
            if (this.state.vehicle !== null && this.state.vehicle.rego !== "" && this.state.vehicle.description !== "") {
                return true
            }
            else {
                return false
    ...
    validateField = (id, value) => {
        ...
        switch (id) {
            ...
            case 'licenceno':
                licenceValid = value !== ""
                fieldErrors.licenceno = licenceValid ? '' : 'Please upload a licensce image'
                break;
            default:
                break;
    ...
    validateForm = () => {
        this.setState({ formValid: this.state.firstnameValid && this.state.lastnameValid && this.state.phoneValid && this.state.licenceValid })
    }
    getAddress = (value) => {
        request({
            method: "GET",
    ...
    loadSwal = () => {
        Swal.fire({
            ...
            background: '#000', onBeforeOpen: () => { Swal.showLoading() }
        })
    }
            ...
            if (this.state.manual) {
                this.setState({
                    vehicle: {
                        ...
                        description: `${document.getElementById('make').value} ${document.getElementById('model').value}`,
                        exp: "",
                        vin: "",
                        engine: "",
                    }
    ...
    toURL = () => {
        this.setState({ sig_64: document.querySelector('canvas').toDataURL("image/jpeg") })
        ...
        Swal.fire({
            ...
            if (result.value) {
                var saveObj = {
                    ...
                    timeOut: this.state.times.outdate,
                    timeIn: ""
                }

                fire.database().ref('/loans').push(saveObj)
