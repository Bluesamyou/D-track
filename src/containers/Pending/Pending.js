...
class Pending extends Component {
    ...
    componentWillMount() {
        fire.database().ref('/loans').on("child_added", (snap) => {
            ...
            if (!snap.val().timeIn) {
                ...
                this.setState({ pendings: [...this.state.pendings, pushObj] })
            }
    ...
    clockIn = (ref) => {
        console.log(ref)
        var changeItem = { ...this.state.pendings[ref], timeIn: this.state.time, testing: true }
        var key = this.state.pendings[ref].key
        console.log(key)
        delete changeItem['key']

        fire.database().ref(`loans/${key}`).update(changeItem)
                    ...
                    this.state.pendings.map((loans, index) => (
                        <div class="card-container">
                            <div class="vehicle-card">
                                ...
                                <Button id="clockIn" onClick={() => this.clockIn(index)}>Mark as returned</Button>
                            </div>
